...
public class CustomerClient {
      ...
      case 1:
        {
          ...
          Set<String> distinctMovieNamesSet = movieNames
            .stream()
            .collect(Collectors.toSet());
          List<String> distinctMovieNamesList = new ArrayList<String>(
            distinctMovieNamesSet
          );
          ...
          Set<String> distinctMovieIdsSet = movieIDs
            .stream()
            .map(movieId ->
              ServerEnum.getEnumNameForValue(movieId.substring(0, 3))
            )
            .collect(Collectors.toSet());
          List<String> distinctMovieIdsList = new ArrayList<String>(
            distinctMovieIdsSet
          );
          ...
          Set<String> distinctSlotSet = movieIDs
            .stream()
            .map(movieId ->
              SlotEnum.getEnumNameForValue(movieId.substring(3, 4))
            )
            .collect(Collectors.toSet());
          List<String> distinctSlotList = new ArrayList<String>(
            distinctSlotSet
          );
          ...
          Movie movie = new Movie(
            movieName,
            ServerEnum.valueOf(movieThreatre).value(),
            SlotEnum.valueOf(slot),
            date
          );
          String response = customer.bookMovieTickets(
            userName,
            movie.getMovieId(),
            movie.getName(),
            numberOfTickets
      ...
      case 3:
        {
          ...
          if (response.equals("")) {
            ...
            List<String> moivesBookedByCustomer = Stream
              .of(response.split(","))
              .map(String::trim)
            ...
            int indexOfDelimiterColon = moivesBookedByCustomer
              .get(index - 1)
              .indexOf(":");
            String[] movieDetails = moivesBookedByCustomer
              .get(index - 1)
              .split(" ");
            String movieName =
              movieDetails[0].substring(0, indexOfDelimiterColon);
            System.out.println(movieName);
            String movieThreatre =
              movieDetails[0].substring(indexOfDelimiterColon + 1);
      ...
      case 4:
        {
          ...
          if (response.equals("")) {
            ...
            String new_movieName = distinctMovieNamesList.get(
              indexForNewMovie - 1
            );
            ...
            System.out.println(
              "Please enter number of tickets to be exchanged for new movie: "
            );
            int numberOfTickets = Integer.parseInt(sc.nextLine().trim());

            Movie old_movie = new Movie(
              old_movieName,
              String.valueOf(ServerEnum.valueOf(old_movieThreatre)),
              SlotEnum.valueOf(old_movieSlot),
              old_movieDate
            );
            Movie new_movie = new Movie(
              new_movieName,
              String.valueOf(ServerEnum.valueOf(new_movieThreatre)),
              SlotEnum.valueOf(new_movieSlot),
              new_movieDate
            );
            response =
              customer.exchangeTickets(
                userName,
                old_movieName,
                old_movie.getMovieId(),
                new_movie.getMovieId(),
                new_movieName,
                numberOfTickets
              );
          }
  ...
  private static void connectToServer(String[] args) {
    try {
      tLocation =
        ServerEnum.getEnumNameForValue(userName.substring(0, 3)).toLowerCase();
      //            String portNum = "";
      //            switch (tLocation) {
      //                case "atwater":
      ...
      customer =
        ServerObjectInterfaceHelper.narrow(ncRef.resolve_str("FrontEnd"));
  ...
  private synchronized List<String> getListFromCommaSeparatedString(
    String response
  ) {
    return Stream
      .of(response.split(","))
