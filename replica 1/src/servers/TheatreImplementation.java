...
public class TheatreImplementation extends DMS_CORBA.CommonInterfacePOA {
  ...
  private final Map<String, Map<String, Integer>> movieData;
  //customerID -> { movieID:count* }
  private final Map<String, List<Booking>> customerBookingData;

  public TheatreImplementation(Logger logger) {
    super();
  ...
  private static boolean isMovieAfterGivenDate(
    String movieID,
    String newMovieID
  ) {
    //        LocalDateTime now = LocalDateTime.now();
  ...
  public void updateMovieCount(
    ...
    idCountMap.put(
      movieID,
      isAdd ? oldCount + noOfBookedTickets : oldCount - noOfBookedTickets
    );
  ...
  public void addCustomerMovieCount(
    ...
    System.out.println("aa - "+customerBookingData);
  }
  ...
  public synchronized String addMovieSlots(
    ...
    if (movieTime.isAfter(afterSevenDaya.toLocalDate())) {
      res = "Cannot add the movie slot after the week";
      ...
      res = "Cannot add the movie slot in past";
    ...
    //
    Boolean isNewSlot = addMovieSlotInHashMap(
      movieName,
      movieID,
      bookingCapacity
    );
    res = isNewSlot ? "Movie slot(s) added" : "Movie slot(s) updated";
  ...
  public synchronized void findNextAvailableSlotAndReturnSlot(
    String customerID,
    String movieName,
    String movieID,
    int oldTicketCount
    ...
    if (idCountMap != null) {
      for (int i = 0; i < listOfMovieID.size(); i++) {
        ...
        if (
          isMovieOnGivenDate(movieID, curMovieId) ||
          isMovieAfterGivenDate(movieID, curMovieId)
        ) {
  ...
  public void findCustomersAndAdjustTimings(String movieName, String movieID) {
    ...
    if (customerBookingData != null) {
      customerBookingData.forEach((customerID, bookingArrayList) -> {
        ...
        //                        }
        //                    });
        Booking oldBooking = bookingArrayList
          .stream()
        ...
        if (oldBooking != null) {
          ...
          //find next
          findNextAvailableSlotAndReturnSlot(
            customerID,
            movieName,
            movieID,
            oldBooking.getCapacity()
          );
        }
      });
    }
  ...
  public synchronized String removeMovieSlots(
    ...
    String res = "";
    if (
      movieData.containsKey(movieName) &&
      movieData.get(movieName).containsKey(movieID)
    ) {
      //
      //            LocalDateTime now = LocalDateTime.now();
      //            String movieDateTime = movieID.substring(4);
      ...
      if (isMovieInPast(movieID)) {
        res =
          "Cannot delete the movie show that occurred from the before the current date";
        utils.Logger.CustomMessage msg = new utils.Logger.CustomMessage(
          "removeMovieSlots",
      ...
      //unsucess
      res =
        "Movie with movieID :" + movieID + " does not exist in the database!";
      utils.Logger.CustomMessage msg = new utils.Logger.CustomMessage(
        "removeMovieSlots",
        "movieID : " + movieID + ", movieName : " + movieName,
        "Error",
  ...
  public String getLocalMoviesShowsAvailability(String movieName) {
    ...
    if (currentMovieData != null) {
      currentMovieData.forEach((k, v) -> {
        sb.append(
          k + " " + v + (count.get() == currentMovieData.size() - 1 ? "" : ",")
        );
        count.getAndIncrement();
      });
    }
  ...
  private boolean checkIfSameSlotInOtherTheatre(
    String customerID,
    String movieID,
    String movieName
  ) {
    ...
    if (
      !customerBookingData.containsKey(customerID) ||
      customerBookingData.get(customerID).size() == 0
    ) return false;
    ...
    for (int i = 0; i < customerBookingList.size(); i++) {
      ...
      if (
        booking.getMovieID().substring(3).equals(movieSlotWithDate) && !booking.getMovieID().substring(0,3).equals(movieID.substring(0,3)) &&
        movieName.equals(booking.getMovieName())
      ) return true;
    }
    return false;
  }
  ...
  public synchronized String bookMovieTickets(
    ...
    if (isMovieInPast(movieID)) {
      res =
        "Cannot book the movie show that occurred from the before the current date";
      utils.Logger.CustomMessage msg = new utils.Logger.CustomMessage(
        "bookMovieTickets",
        "customerID : " +
        customerID +
    ...
    if (clientFromServer.equals(movieInServer)) {
      ...
      if (checkIfSameSlotInOtherTheatre(customerID, movieID, movieName)) {
        res =
          "Cannot book the movie show as the same movie with same slot is booked in another theatre";
        utils.Logger.CustomMessage msg = new utils.Logger.CustomMessage(
          "bookMovieTickets",
          "customerID : " +
          customerID +
      ...
      int countOfTicketFromOtherTheatres = getCountOfTicketFromOtherTheatres(
        customerID
      );
      if (countOfTicketFromOtherTheatres + numberOfTickets <= 3) {
        ...
        res =
          "You can not book more than 3 tickets in other theatres for a week";
        utils.Logger.CustomMessage msg = new utils.Logger.CustomMessage(
          "bookMovieTickets",
          "customerID : " +
          customerID +
  ...
  public synchronized String bookMovieTicketsInLocalServer(
    String customerID,
    String movieID,
    String movieName,
    int numberOfTickets
    ...
    if (numberOfTickets > movieCapacity) {
      res =
        movieID +
        " movie slot does not have enough capacity to book " +
        numberOfTickets +
        " tickets the current theatre.";
  ...
  public int getCountOfTicketFromOtherTheatres(String customerID) {
    if (!customerBookingData.containsKey(customerID)) return 0;
    ...
    int otherTheatreBookingCount = bookings
      .stream()
      .filter(booking ->
        isMovieInFuture(booking.getMovieID()) &&
        !booking.getMovieID().substring(0, 3).equals(clientFromServer)
      )
      .mapToInt(Booking::getCapacity)
      .sum();
  ...
  public synchronized String cancelMovieTickets(
    ...
    if (
      ...
      if (oldBooking == null) {
        ...
        res =
          "The customer does not have " + numberOfTickets + " tickets booked";
        utils.Logger.CustomMessage msg = new utils.Logger.CustomMessage(
          "cancelMovieTickets",
          "customerID : " +
          customerID +
        ...
        bookings =
          bookings
            .stream()
            .filter(booking -> !booking.getMovieID().equals(movieID) && !booking.getMovieName().equals(movieName))
